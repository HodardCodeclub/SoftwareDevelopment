...
def _validate_fossir_dir(ctx, param, value):
...
def _process_name(ctx, param, value):
    path = _get_module_dir(ctx.params['fossir_dir'], ctx.params.get('event'), value)
...
def _get_module_dir(fossir_dir, event, name):
    segments = [fossir_dir, 'modules']
    if event:
...
@click.command()
@click.option('--fossir-dir', envvar='fossir_DIR', metavar='DIR', default='fossir', callback=_validate_fossir_dir,
              is_eager=True, help='Path to the fossir folder. Can be specified via the fossir_DIR env var and '
                                  'defaults to `fossir`')
...
def main(fossir_dir, name, module_dir, event, models, blueprint, templates, controllers, views, model_classes):
    ...
    if models or model_classes:
        ...
        for module_name, class_names in model_classes.iteritems():
            ...
            with open(model_path, 'w') as f:
                write_header(f)
                write(f, 'from fossir.core.db import db')
                write(f, 'from fossir.util.string import format_repr, return_ascii')
    ...
    if blueprint:
        ...
        with open(blueprint_path, 'w') as f:
            write_header(f)
            write(f, 'from fossir.web.flask.wrappers import fossirBlueprint')
            write(f)
            if templates:
                virtual_template_folder = 'events/{}'.format(name) if event else name
                write(f, "_bp = fossirBlueprint('{}', __name__, template_folder='templates',\n\
                      virtual_template_folder='{}')".format(blueprint_name, virtual_template_folder))
            else:
                write(f, "_bp = fossirBlueprint('{}', __name__)".format(blueprint_name))
            write(f)
    if templates:
