...
@click.command()
@click.argument('dbname', required=False, default='fossir')
...
def main(dbname, verbose, apgdiff):
    ...
    models during `fossir db prepare`.

    By default the current database is assumed to be named `fossir`, but it
    can be overridden.  The database uses for comparison will be named
    `fossir_dbdiff` and may not exist. It will be created and dropped
    automatically.

    Since your user may or may not be a database superuser, the database is
    created from a template database named `fossir_template`.  You can create
    it using the following SQL commands:

    \b
        createdb fossir_template
        psql fossir_template -c 'CREATE EXTENSION unaccent;'
        psql fossir_template -c 'CREATE EXTENSION pg_trgm;'
    ...
    _checked_call(verbose, ['createdb', '-T', 'fossir_template', temp_dbname])
    try:
        env_override = {'fossir_CONF_OVERRIDE': repr({'SQLALCHEMY_DATABASE_URI': _build_conn_string(temp_dbname)})}
        _checked_call(verbose, ['fossir', 'db', 'prepare'], env=env_override)
