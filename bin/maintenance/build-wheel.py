...
def git_is_clean_fossir():
    ...
    for cmd in cmds:
        ...
        if rv:
            return False, rv
    rv = subprocess.check_output(['git', 'ls-files', '--others', '--ignored', '--exclude-standard', 'fossir/htdocs'],
                                 stderr=subprocess.STDOUT)
    garbage_re = re.compile(r'^fossir/htdocs/(css|sass|js)/lib')
...
@contextmanager
def patch_fossir_version(add_version_suffix):
...
@click.pass_obj
def build_fossir(obj, deps, add_version_suffix):
    """Builds the fossir wheel."""
    target_dir = obj['target_dir']
    os.chdir(os.path.join(os.path.dirname(__file__), '..', '..'))
    clean, output = git_is_clean_fossir()
    if not clean:
    ...
    clean_build_dirs()
    with patch_fossir_version(add_version_suffix):
