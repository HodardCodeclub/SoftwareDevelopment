...
from fossir.legacy.common.TemplateExec import render as tpl_render
from fossir.legacy.common.utils import isStringHTML
from fossir.util import mdx_latex
from fossir.util.fs import chmod_umask
from fossir.util.i18n import _
from fossir.util.string import render_markdown, sanitize_for_platypus, to_unicode
...
def setTTFonts():
    ...
    if not alreadyRegistered:
        distribution = pkg_resources.get_distribution('fossir-fonts')
        font_dir = os.path.join(distribution.location, 'fossir_fonts')
...
class LatexRunner(object):
    ...
    def run(self, template_name, **kwargs):
        template_dir = os.path.join(get_root_path('fossir'), 'legacy/webinterface/tpls/latex')
        template = tpl_render(os.path.join(template_dir, template_name), kwargs)

        self._dir = tempfile.mkdtemp(prefix="fossir-texgen-", dir=config.TEMP_DIR)
