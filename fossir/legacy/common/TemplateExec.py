...
from fossir.legacy.common.utils import formatDate, formatDateTime, formatDuration, formatTime


# The main template directory
FILTER_IMPORTS = [
    'from fossir.util.json import dumps as j',
    'from fossir.util.string import encode_if_unicode, sanitize_html',
    'from fossir.util.string import html_line_breaks as html_breaks',
    'from fossir.util.string import render_markdown_utf8 as m',
    'from fossir.util.i18n import _'
]


class fossirTemplateLookup(TemplateLookup):
...
def _define_lookup():
    tpl_dir = os.path.join(get_root_path('fossir'), 'legacy/webinterface/tpls')
    return fossirTemplateLookup(directories=[tpl_dir],
...
def mako_call_template_hook(*name, **kwargs):
    from fossir.web.flask.templating import call_template_hook
...
def registerHelpers(objDict):
    ...
    from fossir.util.date_time import format_date, format_datetime, format_human_timedelta, format_number, format_time
    from fossir.util.i18n import ngettext
    from fossir.util.mdx_latex import latex_escape
    from fossir.util.string import safe_upper
