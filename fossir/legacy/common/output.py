...
from fossir.legacy.common.xmlGen import XMLGen
...
from fossir.util.event import uniqueId
...
class outputGenerator(object):
    ...
    def _getRecordCollection(self, obj):
        if obj.is_protected:
            return "fossirSEARCH.PRIVATE"
        else:
            return "fossirSEARCH.PUBLIC"
    ...
    def _generateACLDatafield(self, eType, memberList, objId, out):
        ...
        if objId is not None:
            out.writeTag("subfield", "fossir.%s" % \
                         objId, [["code", "3"]])
    ...
    def _event_to_xml_marc_21(self, event, includeSession=1, includeContribution=1, includeMaterial=1, out=None):
        ...
        out.writeTag("subfield", "fossir." + str(uniqueId(event)), [["code", "a"]])
        out.closeTag("datafield")
    ...
    def _contrib_to_marc_xml_21(self, contrib, include_material=1, out=None):
        ...
        out.writeTag("subfield", "fossir.%s" % uniqueId(contrib), [["code", "a"]])
        out.closeTag("datafield")

        out.openTag("datafield", [["tag", "035"], ["ind1", " "], ["ind2", " "]])
        out.writeTag("subfield", uniqueId(contrib), [["code", "a"]])
        out.writeTag("subfield", "fossir", [["code", "9"]])
        out.closeTag("datafield")
        ...
        out.writeTag("subfield", 'fossir.{}'.format(uniqueId(contrib.event)), [["code", "b"]])
        out.closeTag("datafield")

        out.openTag("datafield",[["tag","970"],["ind1"," "],["ind2"," "]])
        out.writeTag("subfield", 'fossir.{}'.format(uniqueId(contrib)), [["code", "a"]])
        out.closeTag("datafield")
    ...
    def _subcontrib_to_marc_xml_21(self, subcontrib, includeMaterial=1, out=None):
        ...
        out.writeTag("leader", "00000nmm  2200000uu 4500")
        out.openTag("datafield",[["tag","035"],["ind1"," "],["ind2"," "]])
        out.writeTag("subfield", 'fossir.{}'.format(uniqueId(subcontrib)), [["code", "a"]])
        out.closeTag("datafield")
    #
        out.openTag("datafield",[["tag","035"],["ind1"," "],["ind2"," "]])
    ...
    #
        ...
        out.openTag("datafield",[["tag","962"],["ind1"," "],["ind2"," "]])
        out.writeTag("subfield", 'fossir.{}'.format(uniqueId(subcontrib.event)), [["code", "b"]])
        out.closeTag("datafield")

        out.openTag("datafield",[["tag","970"],["ind1"," "],["ind2"," "]])
        confcont = 'fossir.{}'.format(uniqueId(subcontrib))
    ...
    def _attachment_unique_id(self, attachment, add_prefix=True):
        ...
        return 'fossir.{}'.format(unique_id) if add_prefix else unique_id
    ...
    def noteToXMLMarc21(self, note, out=None):
        ...
        out.writeTag('subfield', 'fossir.{}'.format(uniqueId(note)), [['code', '3']])
        out.writeTag('subfield', 'resource', [['code', 'x']])
        out.closeTag('datafield')
