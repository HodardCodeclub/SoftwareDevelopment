...
from fossir.modules.designer import PageSize
from fossir.modules.designer.util import get_inherited_templates
from fossir.modules.events import Event, LegacyEventMapping
from fossir.modules.events.cloning import EventCloner
from fossir.modules.events.fields import EventPersonLinkListField, ReferencesField
from fossir.modules.events.models.events import EventType
from fossir.modules.events.models.references import EventReference
from fossir.modules.events.sessions import COORDINATOR_PRIV_DESCS, COORDINATOR_PRIV_TITLES
from fossir.modules.events.timetable.util import get_top_level_entries
from fossir.util.date_time import format_datetime, format_human_timedelta, now_utc, relativedelta
from fossir.util.i18n import _
from fossir.util.string import is_valid_mail, to_unicode
from fossir.web.flask.util import url_for
from fossir.web.forms.base import fossirForm
from fossir.web.forms.fields import (AccessControlListField, fossirDateField, fossirDateTimeField,
                                     fossirEnumSelectField, fossirLocationField, fossirPasswordField,
                                     fossirProtectionField, fossirRadioField, fossirSelectMultipleCheckboxField,
                                     fossirTagListField, fossirTimezoneSelectField, fossirWeekDayRepetitionField,
...
class EventDatesForm(fossirForm):
    ...
    start_dt = fossirDateTimeField(_("Start"), [InputRequired()], allow_clear=False)
    ...
    start_dt_override = fossirDateTimeField(_("Start"), [Optional()], allow_clear=True,
                                            description=_("Specifying this date overrides the start date displayed "
                                                          "on the main conference page."))
    end_dt_override = fossirDateTimeField(_("End"), [Optional(), LinkedDateTime('start_dt_override', not_equal=True)],
                                          allow_clear=True,
...
class EventClassificationForm(fossirForm):
...
class EventProtectionForm(fossirForm):
    protection_mode = fossirProtectionField(_('Protection mode'),
    ...
    access_key = fossirPasswordField(_('Access key'), toggle=True,
...
class PosterPrintingForm(fossirForm):
    ...
    page_size = fossirEnumSelectField(_('Page size'), enum=PageSize, default=PageSize.A4)
...
class CloneRepeatabilityForm(fossirForm):
    repeatability = fossirRadioField(_('How to repeat'), choices=CLONE_REPEAT_CHOICES, coerce=lambda x: x or None)


class CloneContentsForm(CloneRepeatabilityForm):
    selected_items = fossirSelectMultipleCheckboxField(_('What to clone'))
...
class CloneRepeatUntilFormBase(CloneRepeatOnceForm):
    stop_criterion = fossirRadioField(_('Clone'), [DataRequired()], default='num_times',
                                      choices=(('day', _('Until a given day (inclusive)')),
                                               ('num_times', _('A number of times'))))
    until_dt = fossirDateField(_('Day'), [HiddenUnless('stop_criterion', 'day'), DataRequired()])
...
class CloneRepeatPatternForm(CloneRepeatUntilFormBase):
    week_day = fossirWeekDayRepetitionField(_('Every'))
