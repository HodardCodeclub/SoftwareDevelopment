...
from fossir.modules.events.abstracts.fields import (AbstractField, AbstractPersonLinkListField, EmailRuleListField,
                                                    TrackRoleField)
from fossir.modules.events.abstracts.models.abstracts import EditTrackMode
from fossir.modules.events.abstracts.models.review_questions import AbstractReviewQuestion
from fossir.modules.events.abstracts.models.reviews import AbstractAction, AbstractCommentVisibility
from fossir.modules.events.abstracts.settings import BOACorrespondingAuthorType, BOASortField, abstracts_settings
...
from fossir.web.forms.fields import (EditableFileField, EmailListField, HiddenEnumField, HiddenFieldList,
                                     fossirDateTimeField, fossirEnumSelectField, fossirMarkdownField,
                                     fossirQuerySelectMultipleCheckboxField, fossirQuerySelectMultipleField,
                                     PrincipalListField)
from fossir.web.forms.util import inject_validators
from fossir.web.forms.validators import HiddenUnless, LinkedDateTime, SoftLength, UsedIf, WordCount
...
class BOASettingsForm(fossirForm):
    ...
    extra_text = fossirMarkdownField(_('Additional text'), editor=True, mathjax=True)
    sort_by = fossirEnumSelectField(_('Sort by'), [DataRequired()], enum=BOASortField, sorted=True)
    corresponding_author = fossirEnumSelectField(_('Corresponding author'), [DataRequired()],
...
class AbstractSubmissionSettingsForm(fossirForm):
    ...
    submission_instructions = fossirMarkdownField(_('Instructions'), editor=True,
...
class AbstractReviewingSettingsForm(fossirForm):
    ...
    abstract_review_questions = ReviewQuestionsField(_("Review questions"), question_model=AbstractReviewQuestion,
                                                     extra_fields=[{'id': 'no_score',
                                                                    ...
                                                                    'type': 'checkbox'}])
    reviewing_instructions = fossirMarkdownField(_('Reviewing Instructions'), editor=True,
                                                 description=_("These instructions will be displayed right before the "
                                                               "reviewing form."))
    judgment_instructions = fossirMarkdownField(_('Judgment Instructions'), editor=True,
                                                description=_("These instructions will be displayed right before the "
                                                              "decision box."))
...
class AbstractJudgmentForm(AbstractJudgmentFormBase):
    ...
    judgment = fossirEnumSelectField(_("Judgment"), [DataRequired()], enum=AbstractAction,
...
class AbstractReviewForm(fossirForm):
    ...
    proposed_action = fossirEnumSelectField(_("Proposed Action"), [DataRequired()], enum=AbstractAction)
...
class AbstractReviewingRolesForm(fossirForm):
...
class EditEmailTemplateRuleForm(fossirForm):
...
class AbstractForm(fossirForm):
    title = StringField(_("Title"), [DataRequired()])
    description = fossirMarkdownField(_('Content'), editor=True, mathjax=True)
...
class _SingleChoiceQuerySelectMultipleField(fossirQuerySelectMultipleField):
...
class MultiTrackMixin(object):
    submitted_for_tracks = fossirQuerySelectMultipleCheckboxField(_("Tracks"), get_label='title', collection_class=set)
...
class AbstractCommentForm(fossirForm):
    ...
    visibility = fossirEnumSelectField(_("Visibility"), [DataRequired()], enum=AbstractCommentVisibility,
...
class AbstractReviewedForTracksForm(fossirForm):
    reviewed_for_tracks = fossirQuerySelectMultipleCheckboxField(_("Tracks"), get_label='title', collection_class=set)
