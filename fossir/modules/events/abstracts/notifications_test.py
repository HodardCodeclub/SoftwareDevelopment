...
from fossir.modules.events.abstracts.util import build_default_email_template
...
def test_abstract_notification(mocker, abstract_objects, create_email_template, dummy_user):
    send_email = mocker.patch('fossir.modules.events.abstracts.notifications.send_email')
...
def test_email_content(monkeypatch, abstract_objects, create_email_template, dummy_user):
    def _mock_send_email(email, event, module, user):
        ...
        assert email['subject'] == '[fossir] Abstract Acceptance notification (#314)'
    ...
    monkeypatch.setattr('fossir.modules.events.abstracts.notifications.send_email', _mock_send_email)
