...
from fossir.modules.auth.models.identities import Identity
from fossir.modules.auth.models.registration_requests import RegistrationRequest
from fossir.modules.auth.util import save_identity_info
...
def process_identity(identity_info):
    ...
    if identity is None:
        ...
        raise MultipassException(_('Your fossir profile has been deleted.'))
    else:
        user = identity.user
        if user.is_pending:
    ...
    if user.is_blocked:
        raise MultipassException(_('Your fossir profile has been blocked.'))
...
def login_user(user, identity=None, admin_impersonation=False):
    ...
    :param user: The :class:`~fossir.modules.users.User` to log in to.
