...
from fossir.modules.auth import Identity, logger, login_user
from fossir.modules.auth.forms import (AddLocalIdentityForm, EditLocalIdentityForm, LocalRegistrationForm,
...
from fossir.modules.auth.util import impersonate_user, load_identity_info, register_user, undo_impersonate_user
from fossir.modules.auth.views import WPAuth, WPAuthUser
from fossir.modules.users import User
from fossir.modules.users.controllers import RHUserBase
from fossir.util.i18n import _
from fossir.util.signing import secure_serializer
...
class RHLogin(RH):
    ...
    # Disable global CSRF check. The form might not be an fossirForm
    # but a normal WTForm from Flask-WTF which does not use the same
    # CSRF token as fossir forms use. But since the form has its own
...
class RHRegister(RH):
    ...
    def _process_args(self):
        ...
        if self.provider_name is not None:
            ...
            elif info['fossir_user_id'] is not None or info['provider'] != self.provider_name:
    ...
    def _create_user(self, form, handler):
        ...
        msg = _('You have sucessfully registered your fossir profile. '
...
class RHAccounts(RHUserBase):
    ...
    def _handle_add_local_account(self, form):
        identity = Identity(provider='fossir', identifier=form.data['username'], password=form.data['password'])
...
class LocalRegistrationHandler(RegistrationHandler):
    ...
    def get_identity_data(self, form):
        ...
        return {'provider': 'fossir', 'identifier': form.username.data,
