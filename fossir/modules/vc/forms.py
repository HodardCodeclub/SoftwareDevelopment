...
from fossir.modules.vc.models import VCRoom, VCRoomStatus
from fossir.util.i18n import _
from fossir.web.flask.util import url_for
from fossir.web.forms.base import fossirForm, generated_data
from fossir.web.forms.fields import EmailListField, fossirDateField, fossirRadioField, PrincipalListField
from fossir.web.forms.validators import Exclusive, fossirRegexp, UsedIf
from fossir.web.forms.widgets import JinjaWidget, SelectizeWidget, SwitchWidget
...
class VCRoomFormBase(VCRoomLinkFormBase):
    ...
    name = StringField(_('Name'), [DataRequired(), Length(min=3, max=60), fossirRegexp(ROOM_NAME_RE)],
...
class VCRoomListFilterForm(fossirForm):
    direction = SelectField(_('Sort direction'), [DataRequired()],
                            choices=[('asc', _('Ascending')), ('desc', _('Descending'))])
    abs_start_date = fossirDateField(_('Start Date'), [Optional(), Exclusive('rel_start_date')])
    abs_end_date = fossirDateField(_('End Date'), [Optional(), Exclusive('rel_end_date')])
