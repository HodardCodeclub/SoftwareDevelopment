...
$(document).ready(function() {
    ...
    $(document).on("mouseenter", '[title]:not([title=""]):not(iframe), [data-qtip-html], [data-qtip-oldtitle]:not(iframe)', function(evt) {
        ...
        $target.on('fossir:closeAutoTooltip', function() {
            qtip.qtip('hide');
        });
    });
    ...
    $('body').on('fossir:htmlUpdated', function(evt) {
    ...
    if (match) {
        ...
        var url = match[2] !== undefined
                    ? build_url(fossir.Urls.EventCreation, {event_type: eventType, category_id: categoryId})
                    : build_url(fossir.Urls.EventCreation, {event_type: eventType});
        ajaxDialog({
            url: url,
            title: title
        });
