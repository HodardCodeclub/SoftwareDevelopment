...
function create_favorite_button(user_id) {
    ...
    var active = !!fossir.User.favorite_users[user_id],
...
$(function() {
    $(document).on('favorites.remove-user', function(e, user_id) {
        $.ajax({
            url: build_url(fossir.Urls.FavoriteUserRemove, {fav_user_id: user_id}),
            method: 'DELETE',
            error: handleAjaxError
        }).done(function() {
            $('.favorite-user[data-id=' + user_id + ']').removeClass('active');
            delete fossir.User.favorite_users[user_id];
        });
    }).on('favorites.add-user', function(e, user_id) {
        $.ajax({
            url: fossir.Urls.FavoriteUserAdd,
            method: 'POST',
            dataType: 'json',
            ...
            fossir.User.favorite_users[user_id] = result.users[0];
        });
    });
});
...
type ("FoundPeopleList", ["SelectableListWidget"], {
    ...
    _drawItem: function(pair) {
        ...
        if (peopleData.get('isGroup') || peopleData.get('_type') === 'group') {
            ...
            if (this.showToggleFavouriteButtons && fossirGlobalVars.isUserAuthenticated && peopleData.get('_type') == "Avatar") {
...
type ("SimpleSearchPanel", ["IWidget"], {
   ...
   _createAuthenticatorSearch: function() {
       var self = this;
       if (empty(fossir.Settings.ExtAuthenticators) || !this.allowExternal) {
           return null;
       } else {
            var searchExternalCB = Html.checkbox({});
            $B(searchExternalCB, self.criteria.accessor('search-ext'));
            return [[$T("Users with no fossir account"), searchExternalCB]];
       }
   },
...
type ("UserSearchPanel", ["SimpleSearchPanel"], {
    ...
    _createSearchForm: function() {
        ...
        return fossirUtil.createFormFromMap(fieldList, true);
    },
...
type ("SuggestedUsersPanel", ["IWidget"], {
    ...
    draw: function() {
        ...
        if (this.suggestedUserList.isEmpty()) {
            var message = Html.span({}, $T("There are no suggested users for you at the moment. Why not add some "),
                                        Html.a({href: fossir.Urls.Favorites}, $T("favourites")),
                                        "?");
    ...
    function(onlyOne, includeFavourites, suggestedUsers, selectionObserver, showToggleFavouriteButtons){
        ...
        if (includeFavourites) {
            var favorites = _.values(fossir.User.favorite_users).sort(function(a, b) {
...
type("UserDataPopup", ["ExclusivePopupWithButtons"],
    {
        draw: function() {
            ...
            if (this.grantSubmission) {
                ...
                warning = [Html.span({}, Html.span({style:{fontWeight:'bold'}}, $T('Note:')), $T(' If this person does not already have an fossir account, '), Html.br(),
...
type("UserListWidget", ["ListWidget"],
     {
        ...
        _drawItem: function(user) {
            ...
            if (userData.get('isGroup') || userData.get('_fossil') === 'group') {
                ...
                if (fossirGlobalVars.isUserAuthenticated & this.showToggleFavouriteButtons && userData.get('_type') === "Avatar") {
...
type("UserListField", ["IWidget"], {
    ...
    setUpParameters: function() {
        ...
        this.warning_no_fossir = ($T("Please note that you have added a user that does not exist in fossir.\
    ...
    check: function(user) {
        ...
        if(keys(this.privileges).length>0) {
            ...
            if(!user.get('email')) {
                ...
                fossirRequest('search.users', {email:user.get('email')}, function(result, error) {
                if (!error) {
                    if (result.length == 0) {
                        self.appendMessage(self.warning_no_fossir);
                    }
                }});
            }
        }}
