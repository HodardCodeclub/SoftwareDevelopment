...
type ("RoomBookingCalendarBar", [],
        ...
        function(barInfo, room){
            this.room = room;
            this.startDT = fossirUtil.parseJsonDate(barInfo.startDT);
            this.endDT = fossirUtil.parseJsonDate(barInfo.endDT);
            ...
            this.resvStartDT = barInfo.resvStartDT && fossirUtil.parseJsonDate(barInfo.resvStartDT);
            this.resvEndDT = barInfo.resvEndDT && fossirUtil.parseJsonDate(barInfo.resvEndDT);
        }
        );
...
type ("RoomBookingCalendarData", [],
        ...
        function(options) {
            ...
            this.days.sort(function(day1, day2) {
                return fossirUtil.compare(day1.date, day2.date);
            });
...
type ("RoomBookingSingleRoomCalendarDrawer", ["RoomBookingCalendarDrawer"],
        {
            drawDay: function(day){
                ...
                if (this.room.nonBookableDates) {
                    return Html.span({title: $T("This room cannot be booked for this date"), className: "unavailable"}, Util.formatDateTime(day.date, fossirDateTimeFormats.DefaultHourless, "%Y-%m-%d"));
                } else {
                    ...
                    var link = Html.a({
                        href: build_url(fossir.Urls.RoomBookingBookRoom, {
                            start_date: day.date,
                            roomLocation: this.room.location_name,
                            roomID: this.room.id
                        }),
                        className: 'dateLink ' + dateClass
                    }, Util.formatDateTime(day.date, fossirDateTimeFormats.DefaultHourless, "%Y-%m-%d"));
