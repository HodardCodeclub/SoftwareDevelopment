...
/**
 @namespace Utility functions for fossirUI
*/
...
var fossirUtil = {
    ...
    */
    formatDateTime: function(date, weekDayOption) {
        ...
        case 1:
            return fossir.Data.WeekDays[date.getDay()] + " " + dateText;
        case 2:
            return fossir.Data.WeekDays[date.getDay()].substring(0,3) + " " + dateText;
        case 3:
            return dateText + " (" + fossir.Data.WeekDays[date.getDay()] + ")";
        case 4:
            return dateText + " (" + fossir.Data.WeekDays[date.getDay()].substring(0,3) + ")";
        default:
            return dateText;
        }
    },
    ...
    parameterManager: function() {
        ...
        this.check = function() {
            ...
            entryList.each(function(value) {
                ...
                else if (dataType == 'int' && !(allowEmpty && trim(component.get()) === '') && !fossirUtil.isInteger(component.get())) {
                    error = Html.span({}, $T("Field must be a number"));
                }
                else if (dataType == 'unsigned_int' && !(allowEmpty && trim(component.get()) === '') && (!fossirUtil.isInteger(component.get()) || component.get()<=0)) {
                    error = Html.span({}, "Field must be a positive number");
                }
                else if (dataType == 'non_negative_int' && !(allowEmpty && trim(component.get()) === '') && (!fossirUtil.isInteger(component.get()) || component.get()<0)) {
                    error = Html.span({}, $T("Field must be a positive number"));
                }
                else if (dataType == 'datetime' && !(allowEmpty && trim(component.get()) === '') && !fossirUtil.parseDateTime(component.get())) {
                ...
                else if (dataType == 'time' && !fossirUtil.isTime(trim(component.get()))) {
                ...
                if (exists(error)) {
                    ...
                    if (component.ErrorAware) {
                        ...
                        oList = fossirUtil.markInvalidField(component, error);
