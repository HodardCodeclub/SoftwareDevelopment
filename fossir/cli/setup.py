...
@click.group()
def cli():
    """This script helps with the initial steps of installing fossir"""
...
def list_plugins():
    ...
    for ep in sorted(iter_entry_points('fossir.plugins'), key=attrgetter('name')):
        plugin = ep.load()
...
def create_symlinks(target_dir):
    """Creates useful symlinks to run fossir from a webserver.
    ...
    _copy(os.path.join(root_dir, 'web', 'fossir.wsgi'), os.path.join(target_dir, 'fossir.wsgi'), force=True)


@cli.command()
@click.argument('target_dir')
def create_configs(target_dir):
    """Creates the initial config files for fossir.

    If a file already exists it is left untouched. This command is
    usually only used when doing a fresh fossir installation.
    """
    root_dir, target_dir = _get_dirs(target_dir)
    _copy(os.path.normpath(os.path.join(root_dir, 'fossir.conf.sample')), os.path.join(target_dir, 'fossir.conf'))
...
def wizard(dev):
    """Runs a setup wizard to configure fossir from scratch."""
...
class SetupWizard(object):
    ...
    def run(self, dev):
        ...
        self.config_dir_path = (get_root_path('fossir') if dev else os.path.join(self.root_path, 'etc'))
        self.config_path = os.path.join(self.config_dir_path, 'fossir.conf')
        self._check_configured()
        self._check_directories(dev=dev)
        self._prompt_fossir_url(dev=dev)
        self._prompt_db_uri()
    ...
    def _check_root(self):
        if os.getuid() == 0 or os.geteuid() == 0:
            _error('fossir must not be installed as root.')
            _error('Please create a user (e.g. `fossir`) and run this script again using that user.')
            sys.exit(1)
    ...
    def _prompt_root_path(self, dev=False):
        if dev:
            ...
            if os.path.exists(os.path.join(default_root, '.git')):
                ...
                # this commanad from is `~/dev/fossir/src`, in which case
            ...
            default_root = '/opt/fossir'
        self.root_path = _prompt('fossir root path', default=default_root, path=True,
                                 help='Enter the base directory where fossir will be installed.')
    ...
    def _check_configured(self):
        # Bail out early if fossir is already configured
    ...
    def _check_directories(self, dev=False):
        ...
        if has_existing:
            _warn('The directories marked with a * already exist.  We strongly recommend installing fossir in a new '
    ...
    def _prompt_fossir_url(self, dev=False):
        def _check_url(url):
            data = url_parse(url)
            if not data.scheme or not data.host:
                ...
                _warn('It is recommended to run fossir on a subdomain instead of a subdirectory')
                return False
            return True

        default_url = ('http://127.0.0.1:8000' if dev else 'https://{}'.format(socket.getfqdn()))
        url = _prompt('fossir URL', validate=_check_url, allow_invalid=True,
                      default=default_url,
                      help='fossir needs to know the URL through which it is accessible. '
                           'We strongly recommend using an https:// URL and a subdomain, e.g. '
                           'https://fossir.yourdomain.com')
        self.fossir_url = url.rstrip('/')
    ...
    def _prompt_db_uri(self):
        ...
        if _check_postgres('postgresql:///template1', silent=True):
            default = 'postgresql:///fossir'
        else:
            default = 'postgresql://user:pass@host/fossir'
    ...
    def _prompt_redis_uris(self):
        ...
        self.redis_uri_celery = _prompt('Redis URI (celery)', default='redis://127.0.0.1:6379/0',
                                        ...
                                        help='fossir uses Redis to manage scheduled/background tasks. '
    ...
    def _prompt_emails(self):
        ...
        self.admin_email = _prompt('Admin email', default=self.contact_email, validate=_check_email,
                                   help='If an error occurs fossir sends an email to the admin address. '
                                        'This should be the address of whoever is the technical manager of this '
                                        'fossir instance, i.e. most likely you.')
        self.noreply_email = _prompt('No-reply email',
                                     ...
                                     help='When fossir sends email notifications they are usually sent from the '
                                          'noreply email.')
    ...
    def _prompt_smtp_data(self, dev=False):
        ...
        while True:
            smtp_host = _prompt('SMTP host', default=default_smtp_host,
                                help=('fossir needs an SMTP server to send emails.' if first else None))
    ...
    def _prompt_defaults(self):
        def _get_all_locales():
            # get all directories in fossir/translations
            return os.walk(os.path.join(get_root_path('fossir'), 'translations')).next()[1]
        ...
        self.default_locale = _prompt('Default locale', default='en_GB', list_=_get_all_locales(),
                                      help='Specify the default language/locale used by fossir.')
        ...
        self.default_timezone = _prompt('Default timezone', default=_get_system_timezone(), list_=common_timezones,
                                        ...
                                        help='Specify the default timezone used by fossir.')

    def _prompt_misc(self):
        self.rb_active = _confirm('Enable room booking?',
                                  help='fossir contains a room booking system. If you do not plan to maintain a list '
                                       'of rooms available in your organization in fossir and use it to book them, it '
    ...
    def _prompt_migration(self):
        ...
        self.old_archive_dir = _prompt('Old archive dir', path=True, required=False, validate=_check_path,
                                       allow_invalid=True,
                                       help='If you are upgrading from fossir 1.2, please specify the path to the '
    ...
    def _setup(self, dev=False):
        ...
        config_data = [
            ...
            b'BASE_URL = {!r}'.format(self.fossir_url.encode('utf-8')),
        ...
        if not dev and not create_config_link:
            _echo(cformat('Run %{green!}export fossir_CONFIG={}%{reset} to use your config file')
        ...
        if self.old_archive_dir:
            ...
            _echo(cformat('You can now run %{green!}fossir db prepare%{reset} to initialize your fossir database'))
