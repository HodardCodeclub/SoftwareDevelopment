...
# XXX: Do not import any fossir modules in here!
# See the comment in fossir.cli.core for details.
...
class fossirFlaskGroup(FlaskGroup):
    """
    ...
    def __init__(self, **extra):
        super(fossirFlaskGroup, self).__init__(create_app=_create_app, add_default_commands=False,
                                               add_version_option=False, **extra)
        self._fossir_plugin_commands = None

    def _load_plugin_commands(self):
        # We don't care about `flask.commands` but fossir plugin commands instead
    ...
    def _get_fossir_plugin_commands(self, ctx):
        if self._fossir_plugin_commands is not None:
            return self._fossir_plugin_commands
        try:
            from fossir.core import signals
            from fossir.util.signals import named_objects_from_signal
            ctx.ensure_object(ScriptInfo).load_app()
            cmds = named_objects_from_signal(signals.plugin.cli.send(), plugin_attr='_fossir_plugin')
            rv = {}
            for name, cmd in cmds.viewitems():
                if cmd._fossir_plugin:
                    self._wrap_in_plugin_context(cmd._fossir_plugin, cmd)
                rv[name] = cmd
        except Exception as exc:
            if 'No fossir config found' not in unicode(exc):
                click.echo(click.style('Loading plugin commands failed:', fg='red', bold=True))
                click.echo(click.style(traceback.format_exc(), fg='red'))
            rv = {}
        self._fossir_plugin_commands = rv
        return rv
    ...
    def get_command(self, ctx, name):
        ...
        if rv is not None:
            return rv
        return self._get_fossir_plugin_commands(ctx).get(name)

    def list_commands(self, ctx):
        rv = set(click.Group.list_commands(self, ctx))
        rv.update(self._get_fossir_plugin_commands(ctx))
        return sorted(rv)
