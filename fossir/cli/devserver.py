...
def run_cmd(info, host, port, url, ssl, ssl_key, ssl_cert, quiet, proxy, enable_evalex, evalex_from, reloader_type):
    ...
    os.environ['fossir_CONF_OVERRIDE'] = repr({
        'BASE_URL': url,
    })

    if os.environ.get('WERKZEUG_RUN_MAIN') != 'true':
        print ' * Serving fossir on {}'.format(url)
        if evalex_whitelist:
    ...
    try:
        from fossir.core.config import get_config_path
...
def _make_wsgi_app(info, url, evalex_whitelist, proxy):
    ...
    if proxy:
        app = ProxyFix(app)
    QuietWSGIRequestHandler.fossir_URL_PREFIX = url_data.path.rstrip('/')
    return app


def _make_fossir_dispatcher(wsgi_app, path):
    path = path.rstrip('/')
    if not path:
        # Nothing to dispatch
        return wsgi_app
...
class Debuggedfossir(DebuggedApplication):
    def __init__(self, *args, **kwargs):
        self._evalex_whitelist = None
        self._request_ip = None
        super(Debuggedfossir, self).__init__(*args, **kwargs)
    ...
    def __call__(self, environ, start_response):
        ...
        return super(Debuggedfossir, self).__call__(environ, start_response)


class QuietWSGIRequestHandler(WSGIRequestHandler):
    fossir_URL_PREFIX = ''  # set from outside based on the url path prefix
    ...
    def log_request(self, code='-', size='-'):
        ...
        elif not any(self.path.startswith(self.fossir_URL_PREFIX + x) for x in self.IGNORED_PATH_PREFIXES):
            super(QuietWSGIRequestHandler, self).log_request(code, size)
