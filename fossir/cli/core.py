...
# XXX: Do not import any other fossir modules here!
...
from fossir.cli.util import fossirFlaskGroup, LazyGroup
...
def _get_fossir_version(ctx, param, value):
    if not value or ctx.resilient_parsing:
        return
    import fossir
    message = 'fossir v{}'.format(fossir.__version__)
    click.echo(message, ctx.color)
    ctx.exit()


@click.group(cls=fossirFlaskGroup)
@click.option('--version', '-v', expose_value=False, callback=_get_fossir_version, is_flag=True, is_eager=True,
              help='Show the flask version',)
def cli():
    """
    This script lets you control various aspects of fossir from the
    command line.
    """
...
# if there's no fossir config available or importing fossir is
# broken for other reasons.

@cli.group(cls=LazyGroup, import_name='fossir.cli.setup:cli')
def setup():
    """Setup fossir."""


@cli.group(cls=LazyGroup, import_name='fossir.cli.user:cli')
def user():
    """Manage fossir users."""


@cli.group(cls=LazyGroup, import_name='fossir.cli.event:cli')
def event():
    """Manage fossir events."""


@cli.group(cls=LazyGroup, import_name='fossir.cli.i18n:cli')
def i18n():
    """Perform i18n-related operations."""


@cli.group(cls=LazyGroup, import_name='fossir.cli.database:cli')
def db():
...
def celery(ctx):
    ...
    from fossir.core.celery.cli import celery_cmd
    celery_cmd(ctx.args)
...
def resend_email(paths):
    ...
    the sending to fail.
    """
    from fossir.core.emails import resend_failed_emails_cmd
...
@click.option('--url', '-u', default=None, metavar='URL',
              help='The URL used to access fossir. Defaults to `http(s)://host:port`')
...
def run(info, **kwargs):
    ...
    production setup.
    """
    from fossir.cli.devserver import run_cmd
