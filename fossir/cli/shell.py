...
from fossir.util.date_time import now_utc, server_to_utc
...
def _add_to_context_smart(namespace, info, objects, get_name=attrgetter('__name__'), color='cyan'):
    def _get_module(obj):
        ...
        if segments[0].startswith('fossir_'):  # plugin
            return 'plugin:{}'.format(segments[0])
        elif segments[:2] == ('fossir', 'modules'):
            return 'module:{}'.format(segments[2])
        elif segments[:2] == ('fossir', 'core'):
...
def shell_cmd(verbose, with_req_context):
    try:
        ...
        click.echo(cformat('%{red!}You need to `pip install ipython` to use the fossir shell'))
        sys.exit(1)

    current_app.config['REPL'] = True  # disables e.g. memoize_request
    request_stats_request_started()
    context, info = _make_shell_context()
    banner = cformat('%{yellow!}fossir v{} is ready for your commands').format(fossir.__version__)
    if verbose:
        banner = '\n'.join(info + ['', banner])
