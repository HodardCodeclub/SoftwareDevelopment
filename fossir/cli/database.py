...
import fossir
from fossir.cli.core import cli_group
from fossir.core.db import db
from fossir.core.db.sqlalchemy.migration import PluginScriptDirectory, migrate, prepare_db
from fossir.core.db.sqlalchemy.util.management import get_all_tables
...
def reset_alembic():
    ...
    if current_revs != ['65c079b091bf']:
        ...
        print ('In case of (3), you need to install v1.9.11 and then upgrade the database before updating fossir back '
               'to {}'.format(fossir.__version__))
        sys.exit(1)
...
def _safe_downgrade(*args, **kwargs):
    ...
    if current_app.debug:
        skip_confirm = os.environ.get('fossir_ALWAYS_DOWNGRADE', '').lower() in ('1', 'yes')
        print cformat('%{yellow!}***%{reset} '
