...
SQL_FUNCTION_UNACCENT = '''
    CREATE FUNCTION fossir.fossir_unaccent(value TEXT)
        RETURNS TEXT
    AS $$
    BEGIN
...
def _should_create_function(ddl, target, connection, **kw):
    sql = "SELECT COUNT(*) FROM information_schema.routines WHERE routine_name = 'fossir_unaccent'"
...
def define_unaccented_lowercase_index(column):
    """Defines an index that uses the fossir_unaccent function.
    ...
    use of this index, use this criterion when querying the table::
        ...
        db.func.fossir.fossir_unaccent(db.func.lower(column)).ilike(...)
    ...
    def _after_create(target, conn, **kw):
        ...
        col_func = func.fossir.fossir_unaccent(func.lower(column))
...
def unaccent_match(column, value, exact):
    ...
    return db.func.fossir.fossir_unaccent(db.func.lower(column)).ilike(db.func.fossir.fossir_unaccent(value))
