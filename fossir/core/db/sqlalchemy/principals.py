...
from fossir.util.decorators import classproperty, strict_classproperty
from fossir.util.fossilize import Fossilizable, IFossil, fossilizes
from fossir.util.string import format_repr, return_ascii
from fossir.util.struct.enum import fossirEnum


class PrincipalType(int, fossirEnum):
    user = 1
    local_group = 2
    multipass_group = 3
    email = 4
...
class PrincipalMixin(object):
    ...
    #: This is useful in places where no fossir user exists yet.
