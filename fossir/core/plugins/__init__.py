...
from fossir.modules.users import UserSettingsProxy
from fossir.util.decorators import cached_classproperty, classproperty
from fossir.util.i18n import NullDomain, _
from fossir.util.struct.enum import fossirEnum
from fossir.web.assets import SASS_BASE_MODULES, configure_pyscss
from fossir.web.assets.bundles import LazyCacheEnvironment, get_webassets_cache_dir
from fossir.web.flask.templating import get_template_module, register_template_hook
from fossir.web.flask.util import url_for, url_rule_to_js
from fossir.web.flask.wrappers import fossirBlueprint, fossirBlueprintSetupState
from fossir.web.menu import SideMenuItem
from fossir.web.views import WPJinjaMixin


class PluginCategory(unicode, fossirEnum):
    search = _('Search')
...
class fossirPlugin(Plugin):
    """Base class for an fossir plugin

    All your plugins need to inherit from this class. It extends the
    `Plugin` class from Flask-PluginEngine with useful fossir-specific
    ...
    This class provides methods for some of the more common hooks fossir
    provides. Additional signals are defined in :mod:`~fossir.core.signals`
    ...
    def inject_css(self, name, view_class=None, subclasses=True, condition=None):
        """Injects a CSS bundle into fossir's pages
    ...
    def inject_js(self, name, view_class=None, subclasses=True, condition=None):
        """Injects a JS bundle into fossir's pages
    ...
    def _inject_asset(self, signal, name, view_class=None, subclasses=True, condition=None):
        """Injects an asset bundle into fossir's pages
    ...
    def template_hook(self, name, receiver, priority=50, markup=True):
        ...
        For details see :func:~`fossir.web.flask.templating.register_template_hook`
        """
...
def plugin_url_rule_to_js(endpoint):
    """Like :func:`~fossir.web.flask.util.url_rule_to_js` but prepending plugin name prefix to the endpoint"""
...
def url_for_plugin(endpoint, *targets, **values):
    """Like :func:`~fossir.web.flask.util.url_for` but prepending ``'plugin_'`` to the blueprint name."""
    if '.' in endpoint[1:]:  # 'foo' or '.foo' should not get the prefix
        endpoint = 'plugin_{}'.format(endpoint)
...
def get_plugin_template_module(template_name, **context):
    """Like :func:`~fossir.web.flask.templating.get_template_module`, but using plugin templates"""
    template_name = '{}:{}'.format(current_plugin.name, template_name)
    return get_template_module(template_name, **context)


class fossirPluginEngine(PluginEngine):
    plugin_class = fossirPlugin


class fossirPluginBlueprintSetupState(PluginBlueprintSetupStateMixin, fossirBlueprintSetupState):
    def add_url_rule(self, rule, endpoint=None, view_func=None, **options):
        if rule.startswith('/static'):
            with self._unprefixed():
                super(fossirPluginBlueprintSetupState, self).add_url_rule(rule, endpoint, view_func, **options)
        else:
            super(fossirPluginBlueprintSetupState, self).add_url_rule(rule, endpoint, view_func, **options)


class fossirPluginBlueprint(PluginBlueprintMixin, fossirBlueprint):
    ...
    def make_setup_state(self, app, options, first_registration=False):
        return fossirPluginBlueprintSetupState(self, app, options, first_registration)
...
plugin_engine = fossirPluginEngine()
